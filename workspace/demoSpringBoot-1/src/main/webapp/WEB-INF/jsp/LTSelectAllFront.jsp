...
.article-content {
	...
	<div style="float: right;">
	    ...
	    >
	        <div class="article-item d-flex flex-column align-items-center"> <!-- 將整個文章內容包裝在這個容器中 -->
...
<div class="col-auto ml-auto mx-auto">
	...
	$.each(response.content, function (index, content) {
	    var row = '<div class="article-item">' +
	        '<div class="article-info">' +
	        '<p>文章ID: ' + content.ltId + '</p>' + // 文章ID
	        '<p>標題: ' + content.title + '</p>' + // 標題
	        '<p>發文者ID: ' + content.userId + '</p>' + // 發文者ID
	        '<p>發文時間: ' + content.date + '</p>' + // 發文時間
	        '</div>' +
	        '<hr>' +
	        '<div >' + // 將內容和操作置中
	        '<div class="col-md-12">' +
	        '<div>' +
	        '<p>文章內容: ' + content.content + '</p>' + // 文章內容
	        '</div>' +'<div style="  margin-left: 30%;">' +
	        '<img src="' + content.picture + '" width="300" height="300">' + // 文章圖片
	        '</div>' +
	        '</div>' +
	        '<hr>' +
	        '<div>' + // 將操作置中
	        ...
	        '<div class="row"> <!-- 功能按钮水平排列 -->' +
	            '<form class="col-auto" method="post" action="/ReportinsertFront.controller" onsubmit="handleReportSuccess()">' +
	                ...
	                '</button>' +
	            '</form>' +
	            '<form class="col-auto" method="Get" action="/LTSelectByIdFront.controller/' + content.ltId + '">' +
	                '<button class="btn btn-primary btn-sm" type="submit">修改</button>' +
	            '</form>' +
	            '<form class="col-auto" method="post" action="/LTDeleteByIdFront.controller?ltId=' + content.ltId + '">' +
